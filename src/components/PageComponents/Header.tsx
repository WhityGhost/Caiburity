...
export default function Header({ logo }: { logo: string }) {
  ...
  return (
    ...
    >
      <nav className="lg:w-11/12 2xl:w-4/5 w-full max-w-[1600px] px-6 2xl:px-0 mx-auto py-4 hidden sm:flex items-center justify-between">
        <Link
          href={"/"}
          className="2xl:ml-6 hover:text-cyan-700 hover:dark:text-cyan-500 transition-colors duration-300"
        >
          {logo === "Jigar Sable" ? (
            <FaNodeJs size={28} />
          ) : (
        ...
        <ul className="flex items-center gap-8">
          {navs.map((e, i) => (
            <li key={i}>
              <ScrollLink
                className="hover:text-cyan-700 hover:dark:text-cyan-500 transition-colors capitalize cursor-pointer"
                to={e}
                offset={-60}
                smooth={true}
                duration={500}
          ...
          <span
            ...
            className="hover:bg-gray-100 hover:dark:bg-cyan-700 p-1.5 rounded-full cursor-pointer transition-colors"
          >
      ...
      <nav className="p-4 flex sm:hidden items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <span
            onClick={() => setTheme(theme === "dark" ? "light" : "dark")}
            className="bg-gray-100 dark:bg-cyan-700 p-1.5 rounded-full cursor-pointer transition-colors"
          >
            {theme === "dark" ? <FiSun /> : <FiMoon />}
          </span>
      ...
      <div
        className={`flex w-screen absolute md:hidden top-0 ${
      ...
      >
        ...
        <div className="flex flex-col p-4 gap-5 bg-gray-100/95 backdrop-filter backdrop-blur-sm dark:bg-grey-900/95 w-3/4">
          ...
          <ScrollLink
            ...
            duration={500}
            onClick={() => setNavCollapse(true)}
            className="px-6 py-1.5 rounded-md bg-cyan-600 hover:bg-cyan-700 text-white text-center"
          >
            Contact
          </ScrollLink>
        </div>
